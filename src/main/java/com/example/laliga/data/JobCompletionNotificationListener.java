    ...
    public void afterJob(JobExecution jobExecution) {
        ...
        if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
            ...
            em.createQuery("select distinct m.team2, count(*) from Match m group by m.team2", Object[].class)
             ...
             em.createQuery("select m.FTR, count(*) from Match m group by m.FTR",
             ...
             em.createQuery("select m.FTR2, count(*) from Match m group by m.FTR2",
             Object[].class).getResultList()
             .stream().forEach(e -> {
             Team team = teamData.get((String) e[0]);
             if (team != null)
             team.setTotalLosses((long) e[1]);
             });
